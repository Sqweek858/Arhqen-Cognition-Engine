...
$Build = Join-Path $Root "Build\ACE-ASSET-OPERATIONS"
...
$Sources = @(
    (Join-Path $Root "Tools\AceAssetOperationProbe.cpp"),
...
& $Compiler.Source /nologo /std:c++20 /W4 /WX /EHsc /permissive- /DUNICODE /D_UNICODE /DNOMINMAX /DWIN32_LEAN_AND_MEAN /I"$(Join-Path $Root 'Source\Public')" /Fo"$Objects\" $Sources /link /out:"$(Join-Path $Build 'AceAssetOperationProbe.exe')"
if ($LASTEXITCODE) { throw "Asset operations probe compile failed: $LASTEXITCODE" }
Push-Location $Root
try { & (Join-Path $Build "AceAssetOperationProbe.exe") } finally { Pop-Location }
if ($LASTEXITCODE) { throw "Asset operations probe failed: $LASTEXITCODE" }
Write-Output "PASS|ace_asset_operations_validation"
