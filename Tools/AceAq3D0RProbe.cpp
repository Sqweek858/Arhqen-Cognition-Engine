...
int main()
{
    ...
    if (!hasTruthLabel(afterStep)) pass("debug_off_no_objectkind_labels"); else fail("debug_off_no_objectkind_labels");
    ...
    const auto model = viewport.BuildViewportModel(afterStep, camera, 960.0f, 640.0f);
    if (!model.empty()) pass("dx12_viewport_scene_model_accepts_primitives"); else fail("dx12_viewport_scene_model_accepts_primitives");
