...
if (-not (Test-Path $Probe)) {
    throw "Missing probe: Tools\AceAqCpp2Probe.cpp"
}
...
$Exe = Join-Path $BuildDir "AceAqCpp2Probe.exe"
...
try {
    & $Exe
    if ($LASTEXITCODE -ne 0) {
        throw "AceAqCpp2Probe.cpp failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
...
Write-Host "PASS|ace-aqcpp2_validator|AceAqCpp2Probe.cpp passed against current complete Aquarium core"
