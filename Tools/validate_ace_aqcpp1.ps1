...
$BuildDir = Join-Path $Root "Build\ACE-AQCPP1"
New-Item -ItemType Directory -Force -Path $BuildDir | Out-Null

$Probe = Join-Path $Root "Tools\AceAqCpp1Probe.cpp"
if (-not (Test-Path $Probe)) {
    throw "Missing probe: Tools\AceAqCpp1Probe.cpp"
}
...
$Exe = Join-Path $BuildDir "AceAqCpp1Probe.exe"
...
try {
    & $Exe
    if ($LASTEXITCODE -ne 0) {
        throw "AceAqCpp1Probe.cpp failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
...
Write-Host "PASS|ace-aqcpp1_validator|AceAqCpp1Probe.cpp passed against current complete Aquarium core"
